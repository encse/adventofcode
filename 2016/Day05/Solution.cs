...
namespace AdventOfCode.Y2016.Day05 {
    ...
    class Solution : Solver {
        ...
        string PartOne(string input) {
            var st = "";
            foreach(var hash in CalculateMD5Hash(input.TrimEnd())){
        ...
        public IEnumerable<byte[]> CalculateMD5Hash(string input) {
            var md5 = MD5.Create();
            return from i in Enumerable.Range(0, int.MaxValue)
                   select md5.ComputeHash(Encoding.ASCII.GetBytes(input + i));
        }
    }
}