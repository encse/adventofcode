...
class Solution : Solver {
    ...
    // way to recursively slice up the cube parallel to its edges to smaller and 
    // smaller pieces and decide if the sub-sub cubes at the bottom are accepted 
    // or not. Our Part 2 algorithm follows these rules and returns the 'accepted' 
    ...
    BigInteger Accepted(Rules rules, Cube cube, string state) {
        if (cube.Any(coord => coord.end < coord.begin)) {
            ...
            foreach (var stm in rules[state].Split(",")) {
                ...
                if (TryParseCond(stm, '<', out var cond)) {
                    ...
                    // the cube is split along the right dimension (x) into two:
